const Gallery = () => (
  <section className='max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 py-8'>
    <h2 className='mb-6 lg:mb-12 text-2xl md:text-4xl font-bold text-center'>
      Follow me on Instagram
    </h2>
    <div className='grid grid-cols-2 md:grid-cols-5 gap-4'>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/DKAyZrNhDBl/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-kimchi.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/DJdYbnOR_r6/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-cauli.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/DJKX5OZS4_g/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-kkak.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/DInMd_-x53V/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-jjang.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/DHE09xFSh2m/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-csa.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/C-ePEY-v4GJ/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-jade.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/C62eRRbrzzX/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-dubu.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/C6AcUf0reV0/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-peashoots.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/Cw8iiBFSACV/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-boricha.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/CuFqh4vSLEI/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-bff.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/Cst2cFYv6BR/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-dalgona.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://instagram.com/p/Cr4roIqL6fJ/'>
        <img
          className='max-h-full max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer'
          src='/images/instaPost-beavertonMenu.svg'
          alt=''
          target='_blank'
          rel='noopener noreferrer'
        />
      </a>
      <a
        target='_blank'
        rel='noopener noreferrer'
        href='https://www.instagram.com/p/CrhIMmNvf_Z'>
        <img
          className='max-w-full rounded-lg transition duration-300 ease-in-out hover:scale-105 cursor-pointer object-cover'
          src='/images/instaPost-beaverton.svg'
          alt=''
        />
      </a>
    </div>
  </section>
)

export default Gallery
