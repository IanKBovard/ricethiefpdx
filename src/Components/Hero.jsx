const Hero = () => (
  <div className='max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 py-8'>
    <div className='grid lg:grid-cols-8 lg:gap-x-8 xl:gap-x-12 lg:items-center'>
      <div className='lg:col-span-5'>
        <h1 className='block text-3xl font-bold text-brand-black sm:text-4xl md:text-5xl lg:text-6xl'>
          Korean comfort food
        </h1>
        <h1 className='block text-3xl font-bold text-brand-black sm:text-4xl md:text-5xl lg:text-6xl'>
          made from scratch
        </h1>
        <p className='mt-3 text-2xl text-brand-black'>
          Visit us at Hillsdale Farmers' Market
        </p>
        <p className='text-2xl text-brand-black'>Sundays 9am -1pm</p>
        <div className='mt-5 lg:mt-8 '>
          <a
            className='w-autoc py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-brand-red text-brand-white hover:text-brand-red hover:bg-brand-pink disabled:opacity-50 disabled:pointer-events-none'
            href='/menu'>
            VIEW MENU
          </a>
        </div>
      </div>
      <div className='lg:col-span-3 mt-10 lg:mt-0'>
        <img
          className='w-full rounded-xl'
          src='/images/mayak.svg'
          alt='Hero Image'
        />
      </div>
    </div>
  </div>
)

export default Hero
